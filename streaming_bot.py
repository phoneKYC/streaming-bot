"""
Streaming Bot - Hardened Edition v2.1.0
=======================================
إصلاحات v2.1.0:
  - التبديل من MarkdownV2 إلى HTML (يحل كل أخطاء 400 Bad Request)
  - إضافة query.answer() في كل callbacks (يحل مشكلة تجمد الأزرار)
  - إزالة subprocess.Popen(["true"]) المتزامن
  - إضافة معالج أخطاء عام
  - تقليص الكود عبر دمج التكرارات في _send_or_edit()
  - الحفاظ على كل الوظائف دون تغيير
"""
...
(STATE_STREAM_URL, STATE_SERVER_URL, STATE_STREAM_KEY, STATE_CHANNEL_ID, STATE_CONFIRM) = range(5)

# ---------------------------------------------------------------------------
# نظام الترجمة (HTML - لا حاجة لتهريب معقد)
# ---------------------------------------------------------------------------
TRANSLATIONS: dict[str, dict[str, str]] = {
    "ar": {
        ...
        "setup_saved": "✅ تم حفظ الإعدادات بنجاح! استخدم زر <b>بدء البث</b> لإطلاقه.",
    ...
    "en": {
        ...
        "setup_saved": "✅ Settings saved! Use the <b>Start Stream</b> button to launch.",
...
async def _stream_worker(
    ...
    logger.info(f"🚀 Stream worker started for user {user_id} (type={stream_type})")
...
async def launch_stream(user_id: int, m3u_url: str, server_url: str, stream_key: str) -> bool:
...
async def is_stream_running(user_id: int) -> bool:
    async with _processes_lock:
        sp = _processes.get(user_id)
        return sp is not None and sp.process is not None and sp.process.returncode is None
...
def authorized_only(handler):
    ...
    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE, *args, **kwargs):
        ...
        if not is_user_authorized(user.id):
            # إجابة callback_query لمنع تجمد الزر
...
# ---------------------------------------------------------------------------
# أوامر البث (مُبسطة عبر _send_or_edit)
# ---------------------------------------------------------------------------
@authorized_only
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # الإجابة على أي callback_query معلقة لمنع تجمد الزر
...
def main() -> None:
    ...
    logger.info("🤖 Starting Streaming Bot (hardened v2.1.0 - HTML mode)...")
